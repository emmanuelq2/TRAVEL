...
def test_resolve_location(text, expected):
    assert resolve_location(text) == expected, f"resolve_location({text!r}) should be {expected}"
...
def test_spacy_patterns_airport_code():
    ...
    labels = {p["pattern"]: p["label"] for p in patterns if isinstance(p["pattern"], str)}
